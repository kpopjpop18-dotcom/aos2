...
const DEFAULT_LIBRARY: Photocard[] = [
  seedCard("c1", "common", 340),
  seedCard("c2", "common", 200),
  seedCard("c3", "common", 150),
  seedCard("c4", "rare", 280),
  seedCard("c5", "rare", 50),
  seedCard("c6", "ultra", 300),
  seedCard("c7", "impossible", 15),
];
...
function load(): AOSData {
  ...
  try {
    ...
    // merge library defaults if empty
    if (!parsed.admin?.library?.length) parsed.admin = { ...parsed.admin, library: DEFAULT_LIBRARY };
    return parsed;
  } catch {
    return defaultState();
  }
